...
func FindLocationByDisplayName(config *Config, displayName string) (*Location, error) {
...
func ProcessFzfSelection(config *Config, fzfSelection string) (*SelectionResult, error) {
	...
	location, err := FindLocationByDisplayName(config, displayName)
	if err != nil {
		return nil, err
	}
...
func PrepareCommandInfo(config *Config) []CommandInfo {
...
func RunFzf(config *Config) (*SelectionResult, error) {
	// Prepare command information
	commandInfos := PrepareCommandInfo(config)
